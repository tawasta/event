...
class SurveyUserInputXlsx(models.AbstractModel):
    ...
    def _get_user_input_fnames(self):
        ...
        res = super(SurveyUserInputXlsx, self)._get_user_input_fnames()
        res[_("Event")] = "event_id"
        return res

    def _get_user_input_fname_value(self, user_input, fname):
        res = super(SurveyUserInputXlsx, self)._get_user_input_fname_value(
            user_input, fname
        )
        if fname == "event_id":
            res = user_input.event_id.name or ""
        return res
