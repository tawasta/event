...
class WebsiteEventControllerPrivateEvent(WebsiteEventController):
    ...
    def events(self, page=1, **searches):
        # Kutsu alkuperäistä metodia
        response = super(WebsiteEventControllerPrivateEvent, self).events(page, **searches)

        # Ota event_ids arvot responsesta
        events = response.qcontext.get('event_ids', [])

        # Suodata pois yksityiset tapahtumat
        filtered_events = [event for event in events if not event.is_private_event]

        # Päivitä suodatetut tapahtumat vastaukseen
        response.qcontext['event_ids'] = filtered_events
