...
from odoo.addons.website_portal_track.controllers.event_track \
    import WebsiteEventTrack
from odoo.addons.website_event_track_request_time.controllers.\
...
class WebsiteEventTrack(WebsiteEventTrack):
    ...
    def _get_event_track_proposal_values(self):
        values = super(WebsiteEventTrack, self)\
            ._get_event_track_proposal_values()

        values['request_times'] = request.env['event.track.request.time']\
            .sudo().search([])
