...
class RegistrationInvitation(models.Model):
    ...
    invited_date = fields.Datetime(string='Invitation Date', default=fields.Datetime.now)
    is_used = fields.Boolean(string='Is Used', default=False)
    used_date = fields.Datetime(string='Used Date')
    access_token = fields.Char(string='Access Token', required=True, copy=False, index=True)
