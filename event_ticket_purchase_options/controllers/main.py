...
class EventRegistrationController(WebsiteEventController):
    ...
    def _process_tickets_form(self, event, form_details):
        tickets = super(EventRegistrationController, self)._process_tickets_form(event, form_details)
        
        for ticket in tickets:
            ticket_name = 'nb_register-%s' % ticket['id']
    ...
    def send_invitation(self, **post):
        ...
        if not registration_id:
            return json.dumps({"status": "error", "message": "Registration ID is missing"})
        ...
        invite_tracker = request.env["registration.invitation"].sudo().create({
        ...
        mail_template = request.env.ref('event_ticket_purchase_options.event_invitation_email_template')
        if not mail_template:
            return json.dumps({"status": "error", "message": "Email template not found"})
        ...
        try:
            mail_template.send_mail(
                registration.id, force_send=True,
            ...
            return json.dumps({"status": "error", "message": f"Failed to send invitation: {str(e)}"})

        return json.dumps({"status": "success", "message": "Invitation sent", "invite_id": invite_tracker.id})

    @http.route(['/event/<int:event_id>/invitation/<int:invite_id>/accept'], type='http', auth='public', website=True)
    def accept_invitation(self, event_id, invite_id, access_token=None, **post):
        invitation = request.env['registration.invitation'].sudo().search([('id', '=', invite_id)], limit=1)

        if not invitation or invitation.access_token != access_token:
            return request.render('website.403')

        event = request.env['event.event'].sudo().search([('id', '=', event_id)], limit=1)
        if not event:
            return request.render('website.403')
        ...
        if invitation.is_used:
            ...
            return request.render('event_ticket_purchase_options.event_invitation_form', values)
    ...
    @http.route(['/accept_invitation'], type='http', auth='public', website=True, csrf=True)
    def accept_invitation_form(self, **post):
        ...
        invitation = request.env['registration.invitation'].sudo().search([('id', '=', invite_data['invite_id'])], limit=1)
        form_details = self._sort_form_details(post)

        for invitation.registration_id, form_detail in zip(invitation.registration_id, form_details.values()):
