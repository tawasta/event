...
class EventRegistration(models.Model):
    ...
    def write(self, vals):
        """Tarkistaa, onko ilmoittautuminen vahvistettu ja hylkää muut saman koetyypin ilmoittautumiset."""
    ...
    def _reject_other_registrations(self, confirmed_registration):
        ...
        other_registrations = self.env['event.registration'].sudo().search([
            ('id', '!=', confirmed_registration.id),
            ('partner_id', '=', confirmed_registration.partner_id.id),
            ('registration_survey_id', 'in', confirmed_registration.event_id.survey_ids.ids),
            ('state', 'in', ['draft'])
        ])
