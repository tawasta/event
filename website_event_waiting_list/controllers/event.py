...
class WebsiteEventControllerWaiting(WebsiteEventController):
    ...
    def registration_new(self, event, **post):
        ...
        if event.seats_limited:
            ...
            if event.seats_available < ordered_seats and availability_check and not waiting_list_check:
                if not warning_msg:
                    warning_msg = "You tried to order more tickets than available seats"
                availability_check = False
        if not ticket_dicts:
            return False
        return request.env['ir.ui.view']._render_template("website_event_waiting_list.registration_attendee_details", {'tickets': ticket_dicts, 'event': event, 'availability_check': availability_check, 'waiting_list_check': waiting_list_check, 'warning_msg': warning_msg})
