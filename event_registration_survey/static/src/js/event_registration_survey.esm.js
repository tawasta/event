...
function resetErrors(rootEl) {
    ...
    rootEl.querySelectorAll(".o_survey_error").forEach((el) => el.classList.add("d-none"));
...
function isRequiredQuestionAnswered(questionWrapperEl) {
    ...
    switch (questionType) {
        case "simple_choice_radio":
            return questionWrapperEl.querySelectorAll('input[type="radio"]:checked').length > 0;

        case "multiple_choice":
            return questionWrapperEl.querySelectorAll('input[type="checkbox"]:checked').length > 0;

        case "matrix": {
            const tableEl = questionWrapperEl.querySelector("table");
            const subQuestions = tableEl ? JSON.parse(tableEl.dataset.subQuestions || "[]") : [];
...
export class SurveyRegistrationQuestions extends Interaction {
    ...
    toggleRelatedQuestionVisibility(targetEl, mode) {
        ...
        for (const questionIdToTrigger of questionIdsToTrigger) {
            ...
            const relatedQuestionWrapperEl = this.el.querySelector(`#${CSS.escape(toggledId)}`);
